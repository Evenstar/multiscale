...
\date{}
\maketitle
\abstract{In this paper, we present a model that gives rise to multi-scale adaptive representations of the image signals. This representation improves over dictionary learning in two ways: first, the encoding efficiency is dramatically improved; second, it is a truly multi-scale representation. The proposed model is constructed by stacking several layers of units, where each unit is an adaptive wavelet tight frame. Numerical illustrations are also given to show the potential effectiveness of this new representation.}
